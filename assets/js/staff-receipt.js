...
function receiptClock(value) {
  ...
  if (!/^\d{2}:\d{2}/.test(raw)) return raw || 'Not available';
...
loadBookingDocumentPage = async function (page) {
  ...
  try {
    ...
    const settings = settingsResponse.ok
      ...
      : { center_name: 'AquaLux Auto Spa', contact_number: '' };
    ...
    receipt.innerHTML = `
      ...
      <div class="receipt-footer">
        <span>Generated from live AquaLux SQLite booking and payment records.</span>
        <span>1/1</span>
      </div>
    `;
